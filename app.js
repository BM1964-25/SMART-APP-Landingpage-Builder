...
const staleStatusPatterns = [
  ...
  /Lokaler Node-Server nicht erreichbar/i,
  /Bitte SMART APP & Landingpage Builder\.app starten/i,
  /lokal npm start nutzen/i,
...
function renderApiDiagnostics(diagnostics = null) {
  ...
  const rows = [
    ...
    ["Modell-Quelle", diagnostics.modelSource || "-"],
    ["Models API", diagnostics.modelsStatus ? `${diagnostics.modelsStatus}${diagnostics.modelsStatusText ? ` ${diagnostics.modelsStatusText}` : ""}${diagnostics.modelsOk ? " ok" : " fehler"}` : "-"],
    ["Models Server", diagnostics.modelsResponseServer || "-"],
    ["Models Content-Type", diagnostics.modelsResponseContentType || "-"],
    ["Verfügbare Modelle", Array.isArray(diagnostics.availableModels) && diagnostics.availableModels.length ? diagnostics.availableModels.join(", ") : "-"],
    ...
    ["HTTP-Status", diagnostics.responseStatus ? `${diagnostics.responseStatus}${diagnostics.responseStatusText ? ` ${diagnostics.responseStatusText}` : ""}` : "-"],
    ["Response Server", diagnostics.responseServer || "-"],
    ["Response Content-Type", diagnostics.responseContentType || "-"],
    ["Response Via", diagnostics.responseVia || "-"],
    ["CF-Ray", diagnostics.responseCfRay || "-"],
    ["Request-ID", diagnostics.responseRequestId || "-"],
    ["API-Fehler", diagnostics.apiError || "-"],
    ["Antwortauszug", diagnostics.responseTextPreview || "-"],
